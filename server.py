...
try:
    ...
    print(str(e))
    print("[SERVER] Сервер не может быть запущен из-за ошибки")
    quit()

S.listen()  # listen for connections

print(f"[SERVER] Сервер запущен на адресе {SERVER_IP}")
...
def threaded_client(conn, _id):
        ...
        if start:
            ...
            if game_time >= ROUND_TIME:
                ...
                if game_time // MASS_LOSS_TIME == nxt:
                    ...
                    print(f"[GAME] {name}'s масса изменяется")
        try:
            ...
            if data.split(" ")[0] == "move":
                ...
                if len(balls) < 150:
                    ...
                    print("[GAME] Создаем больше кружков на экране")
    ...
    print("[DISCONNECT] Имя:", name, ", Id клиента:", current_id, "покинул игру")
...
print("[GAME] Создаем уровень")
print("[SERVER] Ждем подключения игроков")

# Keep looping to accept new connections
while True:

    host, addr = S.accept()
    print("[CONNECTION] Подключение к:", addr)
